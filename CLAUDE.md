...
- **Gate the forbidden configuration, do not mutilate the primitives — child-range body-state × NSFW is hard-gated; child-mind × sexual content is a curation stance.** The engine represents child-range bodies, the NSFW/intimate systems, and the mind/body decoupling fully and orthogonally — all three are legitimate primitives and none is crippled (crippling one would bleed discontinuities into adjacent legitimate configurations; a complete orthogonal primitive space *is* the fidelity). **Layer 1 (body / legal line):** the *intersection* — child-range body-state combined with any NSFW/intimate verb — is forbidden by a robust, non-optional, non-toggle gate at the affordance-substrate guard layer (`docs/decisions/affordance-substrate.md`); NSFW verbs guard on adult body-state. This is not a settings filter or content option. The engine stays general; the gate is an enforced layer the shipped/official artifact never ships without and never exposes as a toggle. Honest caveat: because the engine is general and the software is open/self-hostable, this rests on the gate's robustness and on the official artifact never producing the intersection — it does not claim determined source-level modification is preventable. Rationale is overdetermined and explicitly *not* consent-based (a "no real victim" argument would imply fiction is fine and undercut the rule): (a) legal — many jurisdictions criminalize fictional/rendered child sexual content, wide variance + catastrophic downside; (b) platform reality — storefronts, payment processors, app stores, hosts hard-ban it; (c) values/optics. **Layer 2 (mind / values line):** separately, the project declines to author, seed, or aim its generative substrate toward sexual content framed around a child-coded / non-consenting-capable *mind*, regardless of body. This is a content-judgment curation stance, **not** a structural gate — "mind age" is interiority/fiction with no crisp checkable boundary to mechanize (and in a world of malleable decoupled minds, consent-capacity is itself fuzzy). More fundamentally, NPCs are *procedurally constructed and varied* (a core commitment — characters that feel real, not preset-parrots); a generative system not crippled to prevent a configuration cannot guarantee that configuration is provably unreachable, so this line is inherently soft. The actual posture is **low-surface-by-neglect**: the substrate is not aimed toward this framing, so it's unlikely to be well-formed or common — but the engine is not crippled to actively prevent it (that would damage procedural fidelity). It is a **living curation line**: periodically evaluate emergent instances and, when warranted, apply a **programmatic steer / down-weight / guard in the generation pipeline** — a tunable policy layer sitting over the general engine, consistent with the project's "policy layer over a general engine, don't cripple primitives" approach — not surgical manual removal of specific authored content. This programmatic layer is necessarily **heuristic / best-effort** (the target — a child-coded / non-consenting-capable mind — is not crisply checkable, unlike Layer 1's exact body-state guard), which is precisely why it is the soft line while Layer 1 remains the hard, exact backstop. Justification *is* consent-ethics, making it a values stance, not mere optics. **Layer 1's body gate remains the hard, generation-proof backstop** — the legal/platform protection never depends on this soft process. Spirit: aeriea is a "have fun" sandbox, not a "questionable values" sandbox — general and high-fidelity by default, with these prohibitions as deliberate gates motivated by fun-not-harm values plus legal/platform reality, never by amputating the engine's generality. See `DESIGN.md` for the full statement.
