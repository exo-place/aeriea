...
9. **Locomotion upper-body posture — AT-SOURCE FIX LANDED, render bandaid superseded (Not green, awaits user verify).** See `docs/decisions/spine-retarget-world-orientation.md` (the fix) and `docs/decisions/locomotion-upper-body-posture.md` (the now-superseded render workaround). The double-head/hunch was rooted in the INGEST: `tools/motion_ingest.gd` copied per-joint LOCAL rotations over an INVERTED spine topology and dropped BVH Chest3/Chest4, collapsing the whole upper bend onto neck01 (~70-90° head crane). Corrected fix (re-ingested, DB regenerated): the spine/neck/head chain is now solved by orientation-driven closed-form spline-IK over the REAL topology (`root→spine05→…→spine01→neck01→…→head`, all 9 axial joints driven, `bone_count` 17→22), preserving each segment's de-yawed WORLD orientation instead of local rotations. Constraints: torso coronal locked flat (0.0° across all clips), neck coronal passed through (load-bearing for head orientation — hard-locking it reintroduced 27-40° head error), axial twist passed-through-but-per-joint-capped at 25° (natural neck-led twist ≤17° preserved, no dump/synthesis), sagittal free (S emerges in aggregate, ≤2.2° vs source). Re-ingested via `nix build .#motion-assets` over the full 24-clip set (8640 frames): **head world-orientation error ≤6.0° on EVERY clip incl. the previously-corrupt BW/BR/SW/SR/StartStop** (was ~70-90°). The render-side bandaid in `body_rig.gd _apply_motion_matching` (upper body driven from clean idle + procedural arm swing) is LIFTED — motion-matching now drives the full body from the matched frame, restoring real mocap arm swing. KEPT from the bandaid era: matcher loop-safety + the continuity-trap fix (`clip_switch_penalty` 40→3). Legs still foot-locked / planted (no skate; regression guard held). Ingest fails the build on head-error >12° or spine-coronal >4°; `tests/body_motion_matching_test.gd` adds source-fidelity + DB-derived axial regression assertions. **Residual (still open):** the forward↔clip DIRECTION selection (facing-sign) in `motion_matcher §_query_feature` is a SEPARATE open item, unaffected by this fix.
