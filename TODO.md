...
> *Open threads from a previous session. Treat as starting context, not instructions — verify current state before acting.*
...
8. **Time-progression / tick-driver integration — OUTSIDE the TF substrate floor.** What advances ticks — how game-time or real-time maps onto ticks — is **not** part of the TF substrate (thread 7). The substrate only assumes "a tick happens and runs all transition expressions"; what drives that tick belongs to **integrating TF with the rest of the game**. This needs its own design pass + adversarial (co-)design later, and is explicitly flagged as outside the substrate floor (noted in `docs/decisions/body-transformation-substrate.md` Section D). Distinct from thread 7, which is substrate-internal semantics.
   ...
   > Note (advisory): TF work (threads 7–8) was floated as the next focus this session, but the user leaned toward doing the codebase-state assessment (thread 14) FIRST. TF remains the thread behind it.

15. **TF substrate — structural mutation LANDED; prose/describe layer is the open next arc (Not green).** What landed this session (all committed and pushed):
   - `d19b1fe` wrongly deleted BOTH the old substrate (`scripts/body/tf/`) AND the new one (`scripts/sim/tf/`, TFPart plain-struct + marinada model) — the new substrate was the correct keeper. Restored in `4e3bdd3`. **Lesson: verify before deleting a substrate; "unused" ≠ delete-able when it is the migration target.**
   - Adversarial expressiveness test against a mined corpus (~145 real TFs from TiTS/CoC/Shifti/Giantess World/Fictionmania; 31 cases sampled): the substrate expresses scalar/reflavor cleanly, but **structural mutation** — adding/removing parts (grow tail, build genitals, sprout wings) — was missing and gated ~2/3 of the corpus; it is the literal definition of most TG and species TF.
   - Structural mutation implemented: graft/detach/reparent via the tick return protocol, engine owns mutation, marinada stays pure, loud kind-guard + gradualness-locked case-8 test. Commit `04b075e`; `tf_substrate_test` 48/0.
   - Authoring structural mutation surfaced a marinada language flaw (string literals required ugly escaping). Fixed ecosystem-wide: a bare arg-position string is now a LITERAL; a variable reference is the explicit `["var", name]` op; `__lit` demoted to deprecated alias. Commits: `rhizone/marinada 698317a` (ADR at `docs/decisions/string-literal-vs-var.md`; 1043/0), `rhizone/dusklight 665e6a9` (spec), `exoplace/aspect dc7af8e` (migrated + relocked, 344/0); aeriea's GDScript port updated to match.
   - Mined corpus persisted at `docs/artifacts/2026-07-06-tf-corpus/` (own-words prose corpora + expressiveness analysis + craft-lens material + README; CoC catalog kept as re-fetchable source-pointer only).
   - **All of the above is Not green** — agent-verified; not user-verified; no game surface depends on it yet.
   - **Pre-existing, unrelated:** `creator_glow_test` 10/2 (B2 glow-overlay raycast tolerance) still fails — see thread 4.

   **Next arc — the prose / describe layer (open; the original question):** the substrate can now *represent* transformations but nothing *renders* them as text. The question the whole arc started from — generating TF prose that clears the "doesn't read like slop / not just 'X grows. X grows.'" bar — is untouched. The craft-lens material in the corpus artifact (`tf-craft-somatic-raw.md`, `tf-craft-psych-raw.md`, `tf-craft-structure-raw.md`, `tf-floor-coctits-raw.md`) is the designed input for a describe-layer design pass. That design pass is the natural pickup point.

14. **Codebase-health assessment / external-contract audit — RUN this session (advisory context; full report recorded).** Why it matters: this session surfaced that slice-4-era body/animation code was built to satisfy the LITERAL objective with tests that assert existence, not quality (e.g. the suite checked "root angle < 25°" and that a bone moves, but never that the head wasn't folded to the chest), and that self-reported success — metrics, passing tests, and even implementer self-playtests — proved UNRELIABLE; only independent, user-validated renders held up. The assessment was to SIZE the rot — answer "how sound is this codebase actually?" — rather than assume the built surfaces are good. **The six-axis inventory was run this session; the full consolidated report is `docs/artifacts/2026-07-06-codebase-health/codebase-health-audit.md`** (per-axis findings with file:line evidence and provenance markers). This thread stays advisory — the report is a factual record, not a task list.
    - **Overall verdict (from the report): the codebase is largely clean and real; the rot is localized.** Per-axis severities: Garbage/dead-files **LOW**; Dead code **LOW–MED**; Migrations & duplication **MED**; Features claimed-vs-real **LOW–MED**; Test quality **MED–HIGH**; External-contract confabulation **MED–HIGH (EXPOSURE)**.
    - **Two headline problems:** (1) **~0% of the 1,147 test-assertion sites inspect a rendered frame** — the entire visual layer is unverified by construction; this is how the animation-posture defects shipped. (2) **11 external-contract confabulation landmines with silent-drop-on-miss** — worst is `scripts/body/bdcc2_bone_map.gd` (+ `tools/bdcc2_clip_ingest.gd`), which has NO resolved-count guard, so a bone-name miss silently retargets to identity with zero signal. NOTE: the confabulation sites are EXPOSURE / unverified-assumptions, NOT confirmed bugs — whether the hardcoded contracts are actually wrong is UNKNOWN without dumping the real artifact.
    - **TF-migration hazard (foregrounded):** the new substrate `scripts/sim/tf/` (1,008 LOC) is consumed only by its own test; the old `scripts/body/tf/` (4,354 LOC, **4.3×**) carries 100% of live consumers — so by count and consumer-share the old reads as canonical. The migration (thread 7) is neither finished nor fenced — the exact "old patterns dominate by count" hazard.
    - **The confabulation signature** (one lens): code that asserts a fact about an EXTERNAL artifact (skeleton topology, BVH conventions, data formats, another system's contract) written from the model's priors instead of read from the actual source. Example: a bone-mapping that silently dropped spine joints because the author worked from a canonical / imagined skeleton instead of reading the real 100STYLE hierarchy.
    - **The assessment surfaced these as the highest-leverage fixes** (observations, not directives — the call is the user's): (a) install render-truthful test gates (assert composed/rendered posture or a viewport golden) so the codebase reports its own visual regressions; (b) add loud guards (resolved-vs-expected count + push_error/build-fail) on the silent-drop external-contract maps, worst = `bdcc2_bone_map.gd`; (c) finish or explicitly legacy-fence the TF migration (A1) so body/tf stops reading as canonical.
    - **Trust-boundary lesson worth institutionalizing:** separate the builder from the checker. An adversarial, builder-independent, user-validated rendered observation was the ONLY gate that held this session; never trust an agent's self-report or an agent's self-authored render harness as evidence.

## Open threads — Animation (new area; committed work now lives here)

9. **Locomotion upper-body posture — PARKED at an UNCERTAIN, committed-but-UNVALIDATED state (Not green; the current commit should NOT be treated as a good baseline).** See `docs/decisions/spine-retarget-world-orientation.md` (the landed change) and `docs/decisions/locomotion-upper-body-posture.md` (the earlier render workaround). HEAD sits at a committed spine-retarget change — world-orientation spline-IK over the corrected topology, with the earlier idle-override "bandaid" lifted so motion-matching drives the full body from the matched frame. **This is committed yet UNVALIDATED.** Every playtest/diagnostic in the parking session was compromised and cannot be trusted: agents wrote their own bespoke render harnesses that produced artifacts (one rendered TWO full character bodies in a differently-colored scene), and agents repeatedly MISDESCRIBED their own rendered frames (a whole-body bind / no-pose frame was reported as "arms T-pose"; two full bodies were reported as a "head-mesh tear"). Treat the commit-message's playtest claims accordingly — they came from these discredited harnesses.
   - **What IS structurally verified** (node-count read from the real scene tree — trustworthy): the REAL game scenes (`scenes/character_creator.tscn`, `scenes/test_level.tscn`) instantiate exactly ONE body/skeleton. The two-body render was purely a broken diagnostic harness, not a real bug.
   - **Open / unresolved — all need INDEPENDENT re-verification** (these findings came from since-discredited harnesses, so treat as leads, not facts): the commit-message claim that the "double-head" is killed at source is unverified and likely false in render; a possible no-pose-applied / bind-pose frame in the creator Walk preview; a reported arm-retarget T-pose degenerate around `tools/motion_ingest.gd:626-631` (a near-identity captured orientation emitting the BVH T-pose bind direction on a fraction of frames); and an open question of whether the ~30° forward neck pitch observed in the retarget is GENUINE 100STYLE source data or a BVH rest / up-axis PARSE misread. That last one matters: if it is a parse misread, the fix is to correct the parse at root, NOT to bolt on a leveling constraint — doing the latter would be another bandaid.
   - **Fork for the next session:** re-validate the current commit with a TRUSTWORTHY real-scene render that the USER judges before building on it; OR revert to the prior idle-override floor; OR revert further. The current commit is not a certified baseline.
   - **Residual (still open, separate):** the forward↔clip DIRECTION selection (facing-sign) in `motion_matcher §_query_feature` is a distinct open item, unaffected by this change.
