...
14. **Codebase-health assessment / external-contract audit — the LEANING next focus (advisory; scope is itself an open question).** Why it matters: this session surfaced that slice-4-era body/animation code was built to satisfy the LITERAL objective with tests that assert existence, not quality (e.g. the suite checked "root angle < 25°" and that a bone moves, but never that the head wasn't folded to the chest), and that self-reported success — metrics, passing tests, and even implementer self-playtests — proved UNRELIABLE; only independent, user-validated renders held up. Before building more on top, the leaning direction is to SIZE the rot — answer "how sound is this codebase actually?" — rather than assume the built surfaces are good.
    - **Dimensions to gauge** (a general codebase-health inventory, not only the correctness sweep): how much GARBAGE / cruft / scratch / dead files; how much OBSOLETE or dead code (unreferenced, superseded, leftover from abandoned approaches); half-finished or partial MIGRATIONS (old patterns lingering alongside new — the CLAUDE.md "finish migrations before building on top" concern); duplication and near-duplicate implementations; built-but-UNVERIFIED / Not-green features whose real state is unknown; and — as ONE lens among these — the external-contract confabulation signature.
    - **The confabulation signature** (one lens): code that asserts a fact about an EXTERNAL artifact (skeleton topology, BVH conventions, data formats, another system's contract) written from the model's priors instead of read from the actual source. Example found this session: a bone-mapping that silently dropped spine joints because the author worked from a canonical / imagined skeleton instead of reading the real 100STYLE hierarchy.
    - **Shape of the assessment** (advisory): a TARGETED sweep prioritized by exposure — NOT a total line-by-line audit — sizing "how much garbage is there" so the triage call is grounded. Then install render-truthful / ground-truth verification GATES (e.g. tests that assert composed / rendered posture, or validate against the real artifact) so the codebase reports its own regressions going forward.
    - **Process lesson worth institutionalizing:** separate the builder from the checker. An adversarial, builder-independent, user-validated rendered observation was the ONLY gate that held this session; never trust an agent's self-report or an agent's self-authored render harness as evidence.
