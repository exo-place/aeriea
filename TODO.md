...
4. **Character creator + body — SHELVED ("beyond saving for the time being").** Why it matters as context: a large amount of work went into it, but it is not in a resumable state. Do NOT resume without a genuinely different approach — picking up where it left off is explicitly discouraged. The full state lives in `docs/artifacts/creator-saga/SESSION-RECORD.md`: the verbatim rage-list of what's still broken (launcher/creator UI overlap, stranded History/Share/Open, lost region-picking, age floored at 18, coarse rounding, floating text, and more), the design/diagnosis artifact paths, and the meta-learnings. Known/documented there: 2 pre-existing `creator_glow_test` failures (sculpt raycast tolerance). Uncertainty: whether the right move is a rebuild from a new frame or a longer materials/shading/hair art track — undecided, hence shelved.
...
7. **Body/transformation substrate design — semantics still UNDEFINED.** A long design session worked toward a radically-generic body/transformation substrate that blesses almost nothing: everything is uninterpreted author metadata plus author-level patterns. Current state lives in `docs/decisions/body-transformation-substrate.md` (tagged `[CONFIRMED]`/`[REASONED]`/`[OPEN]`), with supporting reasoning in `docs/artifacts/substrate-reasoning/` — read those rather than re-deriving the confirmed invariants. The live gap: the **operational semantics are undefined** — what a metadata value may be; what a predicate may read; `select`'s scope/return/ordering/determinism; an expression's exact inputs, what it may write, when it runs, and evaluation order; what drives the tick; the exact intrinsic structural-order rule and its tie-breaks. No "floor" has been established yet — the model is not pinned to operational semantics nor adversarially tested. The next concrete step the session was heading toward: define each primitive operationally and produce a PASS/FAIL table against a hard-case battery — gradual transformation; an item that pauses the most-recent transformation; two concurrent out-of-step runs of the same transformation on the same target; a cross-part signal like a hormone; selecting one-of-several matches; a non-standard / already-transformed body. A case that forces a new substrate "noun" means it is NOT a floor. **Certified this session (now [CONFIRMED], no longer open forks):** (a) **structure is a tree** — one parent per part; cross-part effects via predicate-guided traversal over metadata + 1-hop structure, NOT graph edges; (b) **cycle/feedback resolution is eval-order only** — a backward/lateral reference reads in-place state, one-tick lag emerges from ordering, and there is **NO stored previous value of any kind** (this RETIRES the prior reasoned per-quantity previous-tick recurrence concession from `synthesis.md` — the no-previous-state invariant wins outright); (c) **a tick runs ALL transition expressions (no scheduler); pause is an authored early-return** (return current value unchanged) and **probabilistic TF is a seeded draw + early-return** — both are expression shapes, not primitives, dissolving the old "pause mechanics" fork. Caveat on (c): the shape still must survive the "pause-most-recent" and "two-out-of-step-runs" hard cases in the PASS/FAIL table, and where each run's progress lives (no blessed "run" noun) is owed to that pass. Hard requirement to preserve: two concurrent out-of-step runs of the same transformation on the same target must be expressible. Process meta-note: this work repeatedly stalled because conventional engine nouns (store, id, resolver, uid, run, cells, paths) kept getting proposed and rejected — the substrate must bless none of them. Useful gate before proposing any mechanism: "does this make the substrate know about a new noun?" If yes, it's wrong by default; derive from metadata + predicate + structure, or ask the user for the missing constraint. The user is the authority on what may be blessed and gates the design before any code. The existing TF MVP (the "discrete-staging interim", e.g. `scripts/body/tf/*`) is what this substrate is meant to eventually supersede, but it is design-gated — no implementation until the user approves the design.
