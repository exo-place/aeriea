...
## Core-body RE-SKINS — REMOVED

An earlier experiment grafted BDCC2 **core-body HEAD** and **LEG** meshes onto aeriea's
MakeHuman skeleton via weight-transfer re-skin (committed `ArrayMesh` `.res` artifacts under
a `reskin/` dir), with per-region base-mesh masking to hide the resulting double-render. It
produced a visibly-wrong second head / set of feet on top of the MakeHuman base and has been
**removed**: the body now renders the pure MakeHuman base mesh directly, unmasked. The
`reskin/` assets, the `head`/`legs` `PartLibrary` slots, the masking code path in `BodyRig`,
and the `tools/bdcc2_{head,body}_reskin.gd` generators are all gone. Only the ACCESSORY
slots below (hair / ears / tails / horns — attached GLBs that sit ON TOP of the base) remain.
